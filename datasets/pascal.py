        ...
        if self.split == 'train':
            ...
            if os.path.exists(filtered_strategy_dir):
                for i, cls in enumerate(self.class_names):
                    start_idx = i * self.examples_per_class
                    end_idx = start_idx + self.examples_per_class
                    for j in range(start_idx, end_idx):
                        cls_images = [img for img in os.listdir(filtered_strategy_dir) if img.startswith(f'aug-{j}-')]
